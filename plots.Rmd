...
```{r}
summary(lm(GPD1 ~ group, meta_norm))
```



```{r boxplots of GPD1 GCNT4 and SLC2A4, fig.height=12}
p1 <- meta_norm %>%
  ggplot(aes(x = forcats::fct_reorder(factor(group), GPD1), y = GPD1, color = group)) +
  geom_boxplot(alpha = 0.5) +
  labs(y = "log10(FPKM)", x = "Group", title = "GPD1")


p2 <- meta_norm %>%
  ggplot(aes(x = forcats::fct_reorder(factor(group), GCNT4), y = GCNT4, color = group)) +
  geom_boxplot(alpha = 0.5) +
  labs(y = "log10(FPKM)", x = "Group", title = "GCNT4")


p3 <- meta_norm %>%
  ggplot(aes(x = forcats::fct_reorder(factor(group), SLC2A4), y = SLC2A4, color = group)) +
  geom_boxplot(alpha = 0.5) +
  labs(y = "log10(FPKM)", x = "Group", title = "SLC2A4")

grid.arrange(p1, p2, p3)
```

```{r annotated heatmap with p-value annotations, warning=FALSE}
...
lgd_pvalue <- Legend(title = "p-value", col_fun = pvalue_col_fun, at = c(0, 1, 2, 3), 
    labels = c("0.2", "0.002", "0.0002", "0.0001"))
